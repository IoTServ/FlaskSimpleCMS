...
class SubmitArticlesForm(Form):
    ...
    price = StringField(u'服务价格', validators=[DataRequired()])
    drivertypes = SelectField(u'证照类型', coerce=int, validators=[DataRequired()])
...
class GravatarForm(Form):
    upload=FileField(u'上传新的头像！(大小不要超过1MB)', validators=[DataRequired()])